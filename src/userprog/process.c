...
process_wait (tid_t child_tid) 
{
	...
	if (!cp) return -1; 
	if (cp->wait) return -1; //системный вызов ожидания уже был выполнен
	cp->wait = true;
	while (!cp->exit) barrier();
	int status = cp->status;
	remove_child_process(cp); //завершился
...
setup_stack (void **esp, const char* file_name, char** save_ptr) 
{
  ...
  if (kpage != NULL) 
	...
	*esp -= sizeof(int8_t); //word-align
	*(uint8_t*)*esp = 0; 


	*esp -= 4;
	*(char*)*esp = '\0'; //null in argv[last]
	...
	*esp -= 4;
	*(char**)*esp = *esp+4;//argv. string begins
